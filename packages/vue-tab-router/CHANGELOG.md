...
- 修复 `DynamicTabsComponent` 内置标签栏分隔线显示异常：分隔线统一由非首个标签的左侧渲染，避免首尾多余分隔线、相邻分隔线粗细不一致以及虚拟标签列表中间标签缺少分隔线的问题。
- 修复 `DynamicContainer` 开启页面 `Transition` 后的滚动恢复时序：组件标签切换时会等待进入动画完成后恢复缓存页面的外层与内部滚动位置，新页面会先同步回到顶部，避免动画期间继承旧页面滚动位置导致内容上下跳动。
- 修复 `mode: "out-in"`、`KeepAlive` 与动态组件切换组合时可能触发 Vue 运行时空节点更新异常的问题，并让过渡驱动节点脱离文档流，避免首页或内容区多出空白高度。
- 修复开启过渡后组件缓存失效的问题：缓存内容层保持稳定，过渡生命周期由独立驱动层处理，切换回已缓存组件时不会重新挂载组件实例。
- 修复已加载异步组件重新打开或重新挂载时仍短暂显示 loading 的问题：现在会复用 Vue async wrapper 已解析的真实组件结果，避免已完成加载的页面切换时再次闪烁加载态。
- 优化组件页面过渡期间的布局稳定性：组件页统一包裹固定尺寸过渡根节点，动画期间仅临时隐藏横向溢出，结束后恢复横向滚动能力。
