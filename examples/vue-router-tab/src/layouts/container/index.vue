<template>
    <div class="container">
        <a-layout :style="{ height: '100%' }">
            <a-layout-header>
                <NavbarComponent :menu-collapsed="menuCollapsed" @logout="logout" @toggle-menu="toggleMenu" />
            </a-layout-header>
            <a-layout class="container-body" :style="{ overflow: 'hidden' }">
                <a-layout-sider :collapsed="siderCollapsed" :class="{
                    'is-mobile-menu-open': isMobile && !menuCollapsed,
                    'is-mobile-menu-closed': isMobile && menuCollapsed,
                }" :style="siderStyle" collapsible :hide-trigger="true" :width="isMobile ? 240 : 260" :collapsed-width="48"
                    @update:collapsed="handleSiderCollapsedChange">
                    <SidebarComponent :selected-keys="selectedMenuKeys" @select-menu="handleSelectMenu" />
                </a-layout-sider>
                <div v-if="isMobile" class="container-menu-mask" :class="{ 'is-open': !menuCollapsed }"
                    @click="menuCollapsed = true" />
                <a-layout>
                    <template v-if="tabsManager.tabs.length > 0">
                        <TabbarComponent />
                    </template>
                    <DynamicBreadcrumbComponent class="container-breadcrumb" :menus="routeMenus"
                        :get-menu-key="getMenuKey" :show-icon="false" />
...
import { DynamicBreadcrumbComponent, useTabsManager } from "@xsbcme/vue-router-tab";
...
const siderStyle = computed(() => {
    ...
    return {
        transform: menuCollapsed.value ? "translateX(calc(-100% - 2px))" : "translateX(0)",
    };
});
const selectedMenuKeys = computed(() => {
    const currentPath = route.path;
    const matched = [...routeMenus]
        .reverse()
        .find(item => currentPath === item.to || currentPath.startsWith(`${item.to}/`));
    return matched ? [matched.key] : [];
});

const getMenuKey = (menu: (typeof routeMenus)[number]) => menu.key;
