<template>
  <a-card :style="{ height: '100%', overflow: 'auto' }" title="Iframe Client">
    <a-space direction="vertical" fill>
      <a-alert type="info">
        这一页演示的是 iframe 页面内部直接使用 `createIframeTabClient()`
        的方式。它不是宿主侧的全局消息监听，也不是手写协议示例。
      </a-alert>
      ...
      <a-alert>
        iframe client 会自动绑定当前 iframe 所属标签，不需要你在页面里自己猜
        tabId。它适合把“打开子页、刷新当前页、更新标题、向来源页签发事件”这类逻辑放回 iframe 页面内部。
      </a-alert>
...
defineTabEvents({
  "iframe-client:hello": data => {
    ...
    iframeLogs.value.unshift(
      `[${new Date().toLocaleTimeString()}] 来源页收到 iframe client 事件：${sourceEventMessage.value}`
    );
