...
这一页开始使用 `views.meta`。如果你还没有跑通 `openTab(viewUrl)`，建议先读 [页面入口与 viewUrl](/views/guide/view-url)。

`views.meta` 负责给页面补充默认标题、图标、打开参数和层级关系。它适合管理“页面说明”，不负责注册组件本身。

## 先分清 modules 和 meta

- `modules` 是页面组件注册表。
- `meta` 是页面说明书，可以让同一个 `viewUrl` 拥有默认标题、图标、单例策略和层级。
- `meta` 不是菜单，也不是 Vue Router 路由表。

只想打开页面时，先配置 `modules` 就够了。需要统一默认标题、图标、缓存策略或面包屑层级时，再加 `meta`。
