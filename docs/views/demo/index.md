...
当前仓库已内置两个示例项目：`examples/vue-tab-router` 用于展示核心包在后台工作台中的完整接入方式，`examples/vue-router-tab` 用于展示 Vue Router 适配包的 route-first 使用方式。本页主要介绍核心包示例，它按“从简单到复杂”的顺序拆成多组小演示。

Vue Router 适配示例包含 `layouts/login` 和 `layouts/workbench`：登录页作为普通 Vue Router 页面存在，不进入标签页；工作台页承载 `RouterTabsContainer`、路由菜单和 route meta 到标签页的同步逻辑。
