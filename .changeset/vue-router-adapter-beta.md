...
"@xsbcme/vue-router-tab": minor
---

新增 Vue Router 标签页适配器第一阶段基础支持。`@xsbcme/vue-router-tab` 作为独立适配包连接 Vue Router 与核心 `TabsManager`，支持通过 `route.meta.tab` 声明标签页标题、图标、缓存、关闭、固定、单例和匹配策略，并初步实现路由切换、标签切换、关闭标签和刷新恢复之间的同步流程。
